...
class RSSFetcher:
    ...
    def fetch_feed(self, feed_url: str, limit: int = 10) -> List[Dict[str, Any]]:
        """抓取单个 RSS feed"""
        try:
            ...
            for entry in feed.entries[:limit]:
                entry_any: Any = entry
                article = {
                    "title": entry_any.get("title", ""),
                    "link": entry_any.get("link", ""),
                    "description": entry_any.get("description", ""),
                    "published": entry_any.get("published", ""),
                    "published_parsed": entry_any.get("published_parsed"),
                    "author": entry_any.get("author", ""),
    ...
    def fetch_all_sources(
        ...
        for key, config in self.sources.items():
            ...
            # 使用源自己的 limit；如未设置则使用参数；最后默认 10
            limit_val = config.get("limit")
            limit: Optional[int] = limit_val if isinstance(limit_val, int) else None
            ...
            if not isinstance(feed_url, str):
                feed_url = str(feed_url) if feed_url is not None else ""
            articles = self.fetch_feed(feed_url, limit)
            results[key] = {
                "name": config["name"],
                "url": config["url"],
    ...
    def add_source(self, key: str, name: str, url: str, category: str = "其他"):
        """添加新的 RSS 源"""
        self.sources[key] = {"name": name, "url": url, "category": category}


# 便捷函数
def fetch_rss_sources() -> Dict[str, Dict[str, Any]]:
    """
    抓取所有配置的 RSS 源
