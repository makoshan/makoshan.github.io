...
import Utils (anyInfix, replace, replaceChecked, sed, sedMany, trim, split, replaceMany, frequency, pairs, delete)
...
guessTagFromShort raw l s = fixedPointSafe (f l) (replace "=" "-" s)
 where f m t = let allTags = nubOrd $ sort m in
       ...
       fixedPointSafe :: (Show a, Eq a, Ord a) => (a -> a) -> a -> a
       fixedPointSafe f0 = go (5000 :: Int) S.empty
         where
           go 0 _ i = i
           go n seen i
             | i `S.member` seen = i
             | otherwise =
                 let i' = f0 i
                 in if i' == i then i else go (n-1) (S.insert i seen) i'
