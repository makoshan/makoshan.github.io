...
import System.Directory (createDirectoryIfMissing, doesFileExist, listDirectory, getModificationTime, doesDirectoryExist, getFileSize, copyFile, removeFile)
     ...
     if not existsOld then do
       ...
       TIO.writeFile target contentsNew
               ...
               if contentsNew /= contentsOld then do tempPath <- emptySystemTempFile ("hakyll-"++template)
                                                     ...
                                                     copyFile tempPath target >> removeFile tempPath
...
-- Gwern.net style prefers a trailing slash for root domains (eg. "https://example.com/").
-- We tolerate missing trailing slashes in legacy content (eg. "https://example.com").
    ...
    Just uri' ->
        ...
        in if null scheme || scheme == "mailto:" || scheme == "irc:" then "" -- skip anchor fragments, emails, IRC
           ...
           then ""
                ...
                Nothing -> ""
                Just auth ->
                    -- Tolerate root domains written without a trailing slash.
                    T.pack $ uriRegName auth
