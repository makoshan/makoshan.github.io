...
htmlDownloadAndParseTitle url =
 ...
 do enabled <- lookupEnv "GWERN_DOWNLOAD_TITLE"
    if enabled /= Just "1" then return "" else do
      CM.cd
...
cleanTitleWithAI title =
  do enabled <- lookupEnv "GWERN_TITLE_CLEANER"
     if enabled /= Just "1" then return title else do
       CM.cd
