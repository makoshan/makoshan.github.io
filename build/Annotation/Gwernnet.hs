...
import Metadata.Format (checkURL, cleanAbstractsHTML, sectionAnonymousRegex, footnoteRegex)
...
gwern md p
        ...
        | '.' `elem` p && not ("/gwern.net_" `isPrefixOf` p) = error $ "Annotation.Gwernnet: attempted to scrape a file, as indicated by a period in its URL? This should be impossible as all file-types should be excluded, and essays aren't allowed to have periods in their name. Look into this. Path was: " ++ p
        | otherwise =
